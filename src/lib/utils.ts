...
export function getMonthDates(year: number, month: number): (Date | null)[][] {
  ...
  const emptyStart = startDayOfWeek === 0 ? 6 : startDayOfWeek - 1;
...
export function getTimeSlots(startHour: number = 7, endHour: number = 21): string[] {
  const slots: string[] = [];
...
export function getScheduleForDay(schedule: import('@/types').ScheduleItem[], dayOfWeek: number) {
  return schedule
    .filter((s) => s.dayOfWeek === dayOfWeek && s.status === 'active')
